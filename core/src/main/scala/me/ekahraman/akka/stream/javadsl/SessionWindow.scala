...
object SessionWindow {
  def apply[T](gap: java.time.Duration, maxSize: Int, overflowStrategy: SessionOverflowStrategy): GraphStage[FlowShape[T, T]] = {
    def fromJavaDuration(duration: java.time.Duration): FiniteDuration = FiniteDuration.apply(duration.toNanos, TimeUnit.NANOSECONDS)
